...
class FullNodeRpcApi:
    ...
    async def get_blockchain_state(self) -> Optional[int]:
        """
        ...
        self.cached_blockchain_state = dict(response)
        return response

    async def get_block(self, header_hash: bytes) -> Optional[FullBlock]:
    ...
    async def get_header(self, header_hash: bytes) -> Optional[Header]:
        header: Optional[Header] = self.service.blockchain.headers.get(
            header_hash, None
        )
        return header

    async def get_unfinished_block_headers(self, height: int) -> List[Header]:
    ...
    async def get_latest_block_headers(self) -> Optional[Dict]:
    ...
    async def get_network_space(self, newer_block_bytes: bytes, older_block_bytes: bytes) -> Optional[int]:
        """
        ...
        return int(network_space_bytes_estimate)

    async def get_unspent_coins(self, puzzle_hash: bytes, header_hash: Optional[bytes]) -> List[CoinRecord]:
        """
